...
//move the per-pixel operations inside the for-loop to a function
//run the function in threads 
//Change some of the variables originally declared in main to become global variables
//this allows the thread function and main be able to access them
int nthreads;
scene_t scene;
//create an float array that store
float array[width][height][3];

//new function that handle the per-pixel operations originally from the main
void *pixelOp(void * numberOfThreads)
{
    Vec3 camera_pos;
    ...
    //modify the starting and ending index here, the increment value also
    //Through modifying the index, split the printing process by multiple threads
    int start = (*(int*) numberOfThreads) * (width / nthreads);
    int end;
    int check = nthreads - 1;
    if ((*(int*) numberOfThreads) == check)
      {
	end = width;
      }
    else
      {
	end = (*(int*) numberOfThreads + 1) * (width / nthreads);
      }
    ...
    for( int px=start; px<end; ++px )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            //comment out the below part and out the appropriate values into the array     
	    ...
	    //put in appropriate values into the array
...
main( int argc, char **argv )
{
  //int nthreads = argc == 2 ? atoi( argv[1] ) : 0;
    ...
    */
    ...
    //create threads according to the passed in arguments
    ...
    for (t = 0; t < nthreads; t++)
      {
	thread_args[t] = t;
	ret = pthread_create(&threads[t], NULL, pixelOp, &thread_args[t]);
	//check if the thread creating process fails
	if (ret)
	  {
    ...
    //wait for each of the thread to complete
    for (t = 0; t < nthreads; t++)
      {
    ...
    //rewrite the print function to print out data from the created array
    //from the newly created array in the above
    int i;
    int j;
    for (i = 0; i < width; i++)
      {
	...
	//after finishing printing one row, put a new line character
	//befor printing the next line
	printf("\n");
      }
